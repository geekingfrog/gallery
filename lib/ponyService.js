module.exports = function (Promise, _, gm, fs, path) {
  var ponyPath = path.resolve(__dirname, '../static/', 'ponies');
  var ponyFiles = fs.readdirAsync(ponyPath).map(function(filename) {
    return path.resolve(__dirname, '../static/', 'ponies', filename);
  });;
  ...
  var addMetadata = Promise.coroutine(function* (file) {
    ...
    return _.assign({}, {file: file}, {name: name}, {size: size}, metadata[name]);
  });
